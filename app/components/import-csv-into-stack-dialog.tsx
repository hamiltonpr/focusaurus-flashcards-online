...
export default function ImportCSVIntoStackDialog({ open, onOpenChange, onImportCards }: ImportCSVIntoStackDialogProps) {
  ...
  const analyzeCSV = (content: string) => {
    try {
      ...
      const firstLine = lines[0].split(detectedSeparator).map((cell) => cell.trim().replace(/^"|"$/g, ""))
      const dataLines = lines.slice(hasHeaders ? 1 : 0, Math.min(6, lines.length))
      const parsedDataLines = dataLines.map((line) =>
      ...
      // Auto-select first two columns as a starting point
  ...
  const parseCSVWithConfig = (content: string): Card[] => {
    ...
    dataLines.forEach((line, index) => {
      const parts = line.split(separator).map((cell) => cell.trim().replace(/^"|"$/g, ""))
  ...
  const handleImport = () => {
    ...
    try {
      ...
      setError("Error creating cards. Please check your configuration.")
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-5xl max-h-[85vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle>Import Cards into Stack {step === "configure" && "- Configure Columns"}</DialogTitle>
        </DialogHeader>
            ...
            <>
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  placeholder="Paste your CSV, TSV, or any delimited content here..."
                  rows={8}
                />
              </div>

              <div className="text-sm text-muted-foreground">
                <strong>Supported formats:</strong> CSV (comma), TSV (tab), semicolon-separated, or any delimited text
              </div>
            </>
          )}
